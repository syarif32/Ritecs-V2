<div class="container-fluid nav-bar px-0 px-lg-4 py-lg-0">
        ...
        <nav class="navbar navbar-expand-lg navbar-light">
            <a href="#" class="navbar-brand p-0">
                <h1 class="text-primary mb-0">
                    <img src="{{ asset('assets/img/logo/logo-text.webp') }}" alt="Logo">
                </h1>
            </a>
            ...
            <div class="collapse navbar-collapse" id="navbarCollapse">
                <div class="navbar-nav mx-0 mx-lg-auto justify-content-center">
                    ...
                    <a href="{{ route('about') }}#visi-misi" class="nav-item nav-link {{ ($title ?? '') === 'About Us' ? 'active' : '' }}">Tentang</a>
                    <a href="{{ route('service') }}" class="nav-item nav-link {{ in_array($title ?? '', ['Membership', 'service']) ? 'active' : '' }}">Layanan</a>
                    
                    <div class="nav-item dropdown">
                        <a href="#" class="nav-item nav-link {{ in_array($title ?? '', ['Buku', 'Jurnal', 'Detail Buku', 'Awardings', 'Members']) ? 'active' : '' }}" data-bs-toggle="dropdown">
                            <span class="dropdown-toggle">Publish</span>
                        </a>
                        <div class="dropdown-menu">
                            <a href="{{ route('buku') }}" class="dropdown-item {{ in_array($title ?? '', ['Buku', 'Detail Buku']) ? 'active' : '' }}">Buku</a>
                            <a href="{{ route('journal') }}" class="dropdown-item {{ in_array($title ?? '', ['Jurnal', 'Detail Jurnal']) ? 'active' : '' }}">Jurnal</a>
                            <a href="{{ route('awardings.index') }}" class="dropdown-item {{ in_array($title ?? '', ['Awardings']) ? 'active' : '' }}">Penghargaan</a>
                            <a href="{{ route('membership.index') }}" class="dropdown-item {{ ($title ?? '') === 'Members' ? 'active' : '' }}">Anggota</a>
                        </div>
                    </div>

                    <div class="nav-item dropdown">
                        <a href="#" class="nav-item nav-link {{ in_array($title ?? '', ['Petunjuk Penulis', 'Layanan Jurnal', 'Contact', 'Layanan HAKI', 'Pusat Pelatihan', '']) ? 'active' : '' }}" data-bs-toggle="dropdown">
                            <span class="dropdown-toggle">Bantuan</span>
                        </a>
                        <div class="dropdown-menu">
                            <a href="{{ route('petunjuk-penulis') }}#petunjuk-penulis" class="dropdown-item {{ ($title ?? '') === 'Petunjuk Penulis' ? 'active' : '' }}">Petunjuk Penulis</a>
                            <a href="{{ route('layanan-journal') }}#layanan-jurnal" class="dropdown-item {{ in_array($title ?? '', ['Service', 'Layanan Jurnal']) ? 'active' : '' }}">Layanan Jurnal</a>
                            <a href="{{ route('training-center') }}#training" class="dropdown-item {{ ($title ?? '') === 'Pusat Pelatihan' ? 'active' : '' }}">Pusat Pelatihan</a>
                            <a href="{{ route('layanan-haki') }}" class="dropdown-item {{ ($title ?? '') === 'Layanan HAKI' ? 'active' : '' }}">Layanan HAKI</a>
                            <a href="{{ route('contact.create') }}#contact" class="dropdown-item {{ ($title ?? '') === 'Contact' ? 'active' : '' }}">Kontak</a>
                        </div>
                    </div>
    ...
    <a href="tel:{{ $global_settings['contact_phone'] ?? '' }}">
        <span class="text-dark">Free: {{ $global_settings['contact_phone'] ?? 'Nomor belum diatur' }}</span>
    </a>
</div>
            </div>
        </nav>
...
<div class="modal fade" id="authModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="authModalLabel" aria-hidden="true">
                            ...
                            <div class="col-lg-6">
                                <div class="modal-body p-4 p-sm-5">
                                    ...
                                    <div class="text-center mb-4">
                                        <img src="{{ asset('assets/img/logo/logo-text.webp') }}" alt="Logo" width="72">
                                    ...
                                    <form method="POST" action="{{ route('login.process') }}">
                                        ...
                                        <div class="form-floating mb-3">
                                            <input type="email" class="form-control @error('email') is-invalid @enderror" id="loginEmail" name="email" placeholder="nama@email.com" value="{{ old('email') }}" required>
    ...
    <span class="position-absolute top-50 end-0 translate-middle-y me-3" 
        ...
        <i class="fa fa-eye text-muted" id="iconLoginPass"></i>
                                    ...
                                    <div class="d-grid">
                                        <a href="{{ route('auth.google.redirect') }}" class="btn btn-light border w-100 py-2 rounded-pill">
                            ...
                            <div class="col-lg-6">
                                <div class="modal-body p-4 p-sm-5">
                                    ...
                                    <form method="POST" action="{{ route('register') }}">
                                        @csrf
                                        <div class="form-floating mb-3">
                                            <input type="text" class="form-control @error('first_name') is-invalid @enderror" id="registerFirstName" name="first_name" placeholder="Nama Depan" value="{{ old('first_name') }}" required>
                                        ...
                                        <div class="form-floating mb-3">
                                            <input type="text" class="form-control @error('last_name') is-invalid @enderror" id="registerLastName" name="last_name" placeholder="Nama Belakang" value="{{ old('last_name') }}">
                                        ...
                                        <div class="form-floating mb-3">
                                            <input type="email" class="form-control @error('email', 'register') is-invalid @enderror" id="registerEmail" name="email" placeholder="nama@email.com" value="{{ old('email') }}" required>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        @if (session('status') || $errors->any() || request()->get('auth') === 'login')
