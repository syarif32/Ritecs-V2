    ...
    <button type="button" 
        ...
        <span class="fw-semibold small">Bantuan & Aturan</span>
                        ...
                        <div class="card-body p-4 p-md-5">
                            ...
                            <form action="{{ route('otp.verify.form') }}" method="POST" class="mt-4">
                                @csrf
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" id="otp" name="otp" required autofocus
                                        class="form-control form-control-lg text-center @if($errors->otp_errors->has('otp')) is-invalid @endif" 
                                        placeholder="_ _ _ _ _ _"
                                        value="{{ old('otp') }}"
                                        style="font-size: 2rem; letter-spacing: 0.5rem; font-weight: 600;">

                                    @if($errors->otp_errors->has('otp'))
                                        <div class="invalid-feedback text-center" style="display: block; font-size: 1rem; margin-top: 10px;">
                                            <strong>{{ $errors->otp_errors->first('otp') }}</strong>
                                        </div>
                                    @endif
                                </div>
                            ...
                            <div class="text-center mt-4">
                                <p class="text-muted small">
                                    ...
                                    <span id="resend-container">
                                        <a href="{{ route('otp.resend', ['userId' => $user_id]) }}" id="resend-link"
                                ...
                                <div class="modal fade" id="manualActivationModal" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <form action="{{ route('otp.request.manual') }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="user_id" value="{{ $user_id }}">
...
<div class="modal fade" id="otpRulesModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            ...
            <div class="modal-body p-4 text-start"> {{-- text-start = Rata Kiri --}}
                
                {{-- LIST NOMOR (Ordered List) --}}
                <ol class="list-group list-group-numbered border-0 mb-0">
                    ...
                    <li class="list-group-item border-0 px-0 pb-3">
                        ...
                        <div class="ms-1 mt-1 text-muted small">
                            ...
                            <ul class="mt-1 mb-0" style="list-style-type: disc;"> {{-- Sub-list bullet --}}
    ...
    {{-- TAMBAHAN: Form Logout Tersembunyi --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let sessionResendTimer = {{ session('resend_timer') ? session('resend_timer') : 'null' }};
