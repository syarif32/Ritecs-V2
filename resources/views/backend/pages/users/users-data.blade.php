              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <span class="pt-1 badge badge-pill badge-{{ $color }}">
                        {{ $status }}
                    </span>
                  </td>
                    ...
                    {{-- Tombol + Member --}}
                      @if(!$user->membership)
                        <a class="dropdown-item text-primary" 
                           href="#" 
                           data-toggle="modal" 
                           data-target="#addMemberModal{{ $user->user_id }}">
                           + Member
                        </a>
                      @else
                ...
                {{-- Modal Tambah Member (spesifik user ini) --}}
                <div class="modal fade" id="addMemberModal{{ $user->user_id }}" tabindex="-1">
                  <div class="modal-dialog modal-md">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title">Tambah Membership - {{ $user->full_name ?? $user->email ?? '(tanpa nama)' }}</h5>
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                      </div>
                
                      <form action="{{ url('/admin/users/' . $user->user_id . '/make-member') }}" method="POST">
                        @csrf
                        <div class="modal-body">
                          <div class="form-group">
                            <label>Nama User</label>
                            <input type="text" class="form-control" value="{{ $user->full_name ?? $user->email ?? '(tanpa nama)' }}" readonly>
                          </div>
                          ...
                          <div class="form-group">
                            ...
                            <input type="date" name="end_date" class="form-control" value="{{ now()->addYear()->format('Y-m-d') }}" required>
                          </div>
                        </div>
